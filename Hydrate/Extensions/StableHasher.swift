...
//
// See https://swift.org/LICENSE.txt for the original license information
...
private func _loadPartialUnalignedUInt64LE(
    ...
    case 7:
        result |= UInt64(
            unsafe p.load(fromByteOffset: 6, as: UInt8.self)
        ) &<< 48
        fallthrough
    case 6:
        result |= UInt64(
            unsafe p.load(fromByteOffset: 5, as: UInt8.self)
        ) &<< 40
        fallthrough
    case 5:
        result |= UInt64(
            unsafe p.load(fromByteOffset: 4, as: UInt8.self)
        ) &<< 32
        fallthrough
    case 4:
        result |= UInt64(
            unsafe p.load(fromByteOffset: 3, as: UInt8.self)
        ) &<< 24
        fallthrough
    case 3:
        result |= UInt64(
            unsafe p.load(fromByteOffset: 2, as: UInt8.self)
        ) &<< 16
        fallthrough
    case 2:
        result |= UInt64(
            unsafe p.load(fromByteOffset: 1, as: UInt8.self)
        ) &<< 8
        fallthrough
    case 1:
        result |= UInt64(
            unsafe p.load(fromByteOffset: 0, as: UInt8.self)
        )
        fallthrough
    case 0:
        return result
...
struct StableHasher {
    ...
    mutating func combine(bytes: UnsafeRawBufferPointer) {
        ...
        do {
            ...
            if c > 0 {
                let chunk = unsafe _loadPartialUnalignedUInt64LE(
                    data,
                    byteCount: c
                )
        ...
        while remaining >= MemoryLayout<UInt64>.size {
            combine(UInt64(littleEndian: unsafe data.load(as: UInt64.self)))
    ...
    mutating func _finalize() -> Int {
        return Int(
            truncatingIfNeeded: _state.finalize(
                tailAndByteCount: _buffer.value
            )
        )
    }
    
    consuming func finalize() -> Int {
        var state = _state
        return Int(
            truncatingIfNeeded: state.finalize(
                tailAndByteCount: _buffer.value
            )
        )
    }
...
protocol StableHashable: Equatable {
    var stableHashValue: Int { get }
    func stableHash(into hasher: inout StableHasher)
}
extension StableHashable {
    static func == (lhs: Self, rhs: Self) -> Bool {
        lhs.stableHashValue == rhs.stableHashValue
    }
    
    var stableHashValue: Int {
        var hasher = StableHasher()
        stableHash(into: &hasher)
        return hasher.finalize()
    }
}
extension StableHasher {
    mutating func combine<H: StableHashable>(_ value: H) {
...
extension String: StableHashable {
    func stableHash(into hasher: inout StableHasher) {
        ...
        cpy.withUTF8 { buffer in
            unsafe hasher.combine(bytes: .init(buffer))
        }
    }
}
...
extension RawRepresentable where Self: StableHashable, RawValue: StableHashable {
    func stableHash(into hasher: inout StableHasher) {
